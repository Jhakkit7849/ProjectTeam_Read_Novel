...
// ✅ ลิสต์นิยายของผู้ใช้ที่ล็อกอิน (ผลงานของฉัน)
...
// ✅ ดึงนิยายเพื่อแก้ไข (เฉพาะเจ้าของ)
router.get("/manage/:id", requireAuth, async (req, res) => {
  const r = await q(`SELECT * FROM novels WHERE id=$1 AND author_id=$2`, [
    req.params.id,
    req.user.id,
  ]);
  if (!r.rowCount)
    return res.status(403).json({ message: "Not found or no permission" });
...
router.post("/", requireAuth, async (req, res) => {
  ...
  if (!["original", "fanfic"].includes(type))
    return res.status(400).json({ message: "type must be original or fanfic" });
  if (!ratingSet.includes(rating))
    return res.status(400).json({ message: "invalid rating" });

  // อัปเกรด role ครั้งแรกที่สร้างนิยาย
  let newToken = null;
  if (req.user.role === "reader") {
    await q(`UPDATE users SET role='writer' WHERE id=$1 AND role='reader'`, [
      req.user.id,
    ]);
    const ur = await q(
...
router.put("/:id", requireAuth, async (req, res) => {
  ...
  for (const name of tags) {
    ...
    await q(
      `INSERT INTO novel_tags(novel_id, tag_id) VALUES ($1,$2) ON CONFLICT DO NOTHING`,
      [novel.id, t.rows[0].id]
    );
  }
  res.json(novel);
...
router.get("/", optionalAuth, async (req, res) => {
  ...
  if (rating) {
    params.push(rating);
    where.push(`n.rating=$${params.length}`);
  }
  if (tag) {
    params.push(tag);
    where.push(
      `EXISTS(SELECT 1 FROM novel_tags nt JOIN tags t ON t.id=nt.tag_id WHERE nt.novel_id=n.id AND t.slug=$${params.length})`
    );
  }
  const order =
    sort === "top" ? "n.fav_count DESC NULLS LAST" : "n.created_at DESC";
